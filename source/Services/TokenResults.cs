...
/// </summary>
/// <remarks>
/// The store used to answer with a bare <c>false</c> for "locked out" and
/// "that is not even shaped like a token" alike, and the controller had no
/// choice but to render both as a 403 claiming the token was locked. The
/// caller needs to be able to tell those apart to answer honestly — which is
/// not the same as telling the <em>client</em> apart, and deliberately so:
/// <see cref="Invalid"/> covers an unknown token, a wrong one, an expired
/// one, one already spent and one presented from the wrong address, because
/// distinguishing those to a caller would confirm a guess.
/// </remarks>
